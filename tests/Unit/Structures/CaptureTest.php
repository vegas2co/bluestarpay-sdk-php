...
final class CaptureTest extends StructureTestCase
{
    protected $struct = BlueStar\Payments\Structures\Capture::class;

    public function test_type() {
        $struct = new $this->struct;
        $this->assertEquals("Capture", $struct->type() );
    }
    ...
    public function test_origionalTransaction() { $this->full( "originalTransaction", Structures\Transaction::class, true);}
    public function test_capturedTransaction() { $this->full( "capturedTransaction", Structures\Auth::class, true); }
}
